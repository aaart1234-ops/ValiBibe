import { useState } from 'react'
import {
    Box,
    Typography,
    Divider,
    LinearProgress,
    Stack,
    Chip,
    Menu,
    MenuItem,
} from '@mui/material'
import { Note } from '../noteApi'
import dayjs from 'dayjs'
import ArchiveIcon from '@mui/icons-material/Archive'
import { useLongPress } from '@/features/note/hooks/useLongPress'

interface NoteRowProps {
    note: Note
    onRequestArchive?: (note: Note) => void // сделали опциональными
    onRequestDelete?: (note: Note) => void // чтобы можно было использовать без меню
    isArchiveView?: boolean
}

const NoteRow = ({
                     note,
                     onRequestArchive,
                     onRequestDelete,
                     isArchiveView,
                 }: NoteRowProps) => {
    const [anchorEl, setAnchorEl] = useState<null | HTMLElement>(null)

    const handleOpenMenu = (e: any) => setAnchorEl(e.currentTarget as HTMLElement)
    const handleClose = () => setAnchorEl(null)

    const enableMenu = Boolean(onRequestArchive || onRequestDelete)

    const longPressHandlers = enableMenu
        ? useLongPress({
            onLongPress: handleOpenMenu,
            delay: 600,
        })
        : ({} as Record<string, unknown>)

    return (
        <Box mb={2} sx={{ width: '100%' }} {...longPressHandlers}>
            <Stack
                direction="row"
                justifyContent="space-between"
                alignItems="center"
                flexWrap="wrap"
                gap={1}
            >
                <Typography variant="subtitle1" fontWeight={500}>
                    {note.title}
                </Typography>

                <Stack direction="row" spacing={1} alignItems="center">
                    {note.archived && (
                        <Chip
                            icon={<ArchiveIcon />}
                            label="В архиве"
                            size="small"
                            color="default"
                            clickable={false}
                        />
                    )}
                    <Typography variant="caption" color="text.secondary">
                        {dayjs(note.created_at).format('D MMM YYYY')}
                    </Typography>
                </Stack>
            </Stack>

            <Box mt={1}>
                <Typography variant="caption" color="text.secondary">
                    Уровень запоминания: {note.memoryLevel}%
                </Typography>
                <LinearProgress
                    variant="determinate"
                    value={note.memoryLevel}
                    sx={{ height: 6, borderRadius: 4, mt: 0.5 }}
                    color={
                        note.memoryLevel < 40 ? 'error' : note.memoryLevel < 70 ? 'warning' : 'success'
                    }
                />
            </Box>

            <Divider sx={{ mt: 2 }} />

            {/* Меню действий (только если прокинуты коллбеки) */}
            <Menu anchorEl={anchorEl} open={Boolean(anchorEl) && enableMenu} onClose={handleClose}>
                {onRequestDelete && (
                    <MenuItem
                        onClick={() => {
                            handleClose()
                            onRequestDelete(note)
                        }}
                    >
                        Удалить
                    </MenuItem>
                )}
                {onRequestArchive && (
                    <MenuItem
                        onClick={() => {
                            handleClose()
                            onRequestArchive(note)
                        }}
                    >
                        {isArchiveView ? 'Вернуть из архива' : 'Архивировать'}
                    </MenuItem>
                )}
            </Menu>
        </Box>
    )
}

export default NoteRow
